...
from sim_mac_w_mix_and_hydration import StaticBoundary, DynamicBoundary, RigidBody, SoftBody, MpmSim
...
from sim_mac_w_mix_and_hydration import NeoHookean, StVK_with_Hecky_strain, visco_StVK_with_Hecky_strain, \
...
def test_sim():
    ...
    flour_par = flour_par - flour_par.mean(axis=0) + np.array([0.5, 0.49, 0.5])
    ...
    cup_mesh.vertices *= np.array([0.3, 1.3, 0.3])
    ...
    # indices = np.random.choice(cup_mesh.vertices.shape[0], size=1000, replace=False)  
    # cup_par = cup_mesh.vertices[indices]
    # cup_mesh.vertices
    # cup_material = NeoHookean(1e3, 0.01, False)
    # cup_color = np.array([0.15, 0.15, 0.15])
    # cup_mac = SoftBody(
    #     cup_par, cup_material, cup_color, 0.0, 0.0, 1.0)
    



    water_material = NeoHookean(3e-2, 0.45, True)
    water_par = np.random.rand(15000, 3) * 0.045 * np.array([1, 1.5, 1])
    water_par = water_par - water_par.mean(axis=0) + cup_pos
