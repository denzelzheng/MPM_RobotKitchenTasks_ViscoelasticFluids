...
def make_matrix_from_diag(d):
    return ti.Matrix([[d[0], 0.0, 0.0], [0.0, d[1], 0.0], [0.0, 0.0, d[2]]], dt=T)
...
class NeoHookean_VonMise(Material):
    def __init__(self, E: float = 5e3, nu: float = 0.2, yield_stress: float = 1e3, fluid: bool = False) -> None:
        super().__init__()
        self.mu, self.lam = E / (2 * (1 + nu)), E * \
            nu / ((1+nu) * (1 - 2 * nu))
        self.fluid = fluid
...
class DynamicBoundary(Boundary):
    def __init__(self, 
            ...
            self.initial_center = vecs(3, T, shape=())
    ...
    def update_vertices(self):
        ...
        for i in self.vertices:
            self.vertices[i] = ti.cast(
                transform3_pos_ti(self.init_vertices[i]-self.initial_center[None], self.pos[None], self.quat[None])+self.initial_center[None], 
                self.vertices.dtype)

    @ti.func
    def collider_v(self, pos_world, dt):
        pos_mesh = inv_transform3_pos_ti(pos_world-self.initial_center[None], self.pos[None], self.quat[None])+self.initial_center[None]
        pos_world_new = transform3_pos_ti(pos_mesh-self.initial_center[None], self.target_pos[None], self.target_quat[None])+self.initial_center[None]
    ...
    def transform_pos_from_world_to_mesh(self, pos_world):
        return inv_transform3_pos_ti(pos_world-self.initial_center[None], self.pos[None], self.quat[None])+self.initial_center[None]
    
    @ti.func
    def transform_pos_from_mesh_to_world(self, pos_mesh):
        return transform3_pos_ti(pos_mesh-self.initial_center[None], self.pos[None], self.quat[None])+self.initial_center[None]
    ...
    def set_target(self, target_pos: np.ndarray, target_quat: np.ndarray, initial_center:np.ndarray):
        self.target_pos.from_numpy(target_pos)
        self.target_quat.from_numpy(target_quat)
        self.initial_center.from_numpy(initial_center)
...
class MpmSim:
    ...
    def reset(self):
        ...
        # self.x_time_series = []
        # self.x_color_time_series = []

        # fileds
        self.x: Optional[vecs] = None
        self.valve_pos: Optional[scalars] = None
        ...
        self.hydra_p_c: Optional[vecs] = None   
        ...
        self.grid_hydra_p_c: Optional[scalars] = None
    ...
    def init_system(self):
        ...
        if self.n_soft_pars:   
            ...
            self.valve_pos = scalars(T, shape=())
    ...
    def substep(self):
        ...
        self.valve_pos[None] = self.valve
        self.init_step()
        if self.n_lag_verts:
    ...
    def update_scene(self):
        ...
        if self.x:
            ...
            self.np_x = self.x.to_numpy()
            self.np_x_color = self.x_color.to_numpy()
            ...
            for body_id in unique_body_ids:
                ...
                x_by_body_id[body_id] = self.np_x[mask]
                x_color_by_body_id[body_id] = self.np_x_color[mask]
        ...
        if self.n_static_bounds:
            for b in self.static_bounds:
                self.scene.mesh(b.vertices, b.faces, color=(0.3, 0.5, 0.3))
        if self.n_dynamic_bounds:
            for b in self.dynamic_bounds:
                self.scene.mesh(b.vertices, b.faces, color=(0.15, 0.15, 0.15))
        if self.n_lag_verts:
    ...
    def P2G(self):
            ...
            for p in self.x:  # Particle state update and scatter to grid (P2G)
                ...
                for i in ti.static(range(len(self.materials))):
                    if i == self.x_body_id[p]:
                        ...
                        if (self.hydrating_efficacy[i] > 0.5 and self.x[p][1] > self.valve_pos[None]):   # water 
                            SIM = False

                # if self.x[p][1] > self.valve_pos[None]:
                #     SIM = False
                ...
                for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
                    ...
                    self.grid_hydra_p_c[position] += particle_phase * (1.0 - 1.0 * self.hydration[p])# consider hydra consuming
        ...
        if ti.static(self.n_lag_verts):
            for p in self.x_lag:
                ...
                # self.v_lag[p] += self.gravity * self.dt
                for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
                    offset = ti.Vector([i, j, k])
        ...
        if ti.static(self.n_soft_pars):
            for p in self.x:  # grid to particle (G2P)
                ...
                for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
                    ...
                    g_h_p_c = self.grid_hydra_p_c[position]
                ...
                for q in ti.static(range(self.n_phases)):
                    ...
                    new_h_p_c_sum += new_h_p_c[q]
                new_c = new_c / new_p_c_sum   
                self.p_c[p] = new_p_c / new_p_c_sum
                self.hydra_p_c[p] = new_h_p_c / new_h_p_c_sum 
    ...
    def hydration_update(self):
        if ti.static(self.n_soft_pars):
            for p in self.x:
                ...
                hydrated_concentration = 0.0
                for q in ti.static(range(self.n_phases)): 
                    hydra_concentration += self.hydra_p_c[p][q] * self.hydrating_efficacy[q]

                    hydrated_concentration += self.hydra_p_c[p][q] * self.hygroscopicity[q]
                ...
                for q in ti.static(range(len(self.materials))):
                    if q == self.x_body_id[p] and self.hygroscopicity[q] > 0.5:
                        water_absorb = 1.0
                    if q == self.x_body_id[p] and self.hydrating_efficacy[q] > 0.5:
                        water_consume = 1.0
                ...
                if (hydra_concentration >= self.critical_concentration) and water_absorb > 1e-5:
                    ...
                    self.x_color[p] += (self.hydration[p] - prev_hydration) * (hydration_color - self.x_color[p]) *  self.coloring_hydra_alpha

                if (hydrated_concentration >= self.critical_concentration) and water_consume > 1e-5:
